...
import { LifecycleStep, LifecycleStepDetails, Netbook, NetbookReport } from '@/lib/types';
...
function NetbookDetailsSkeleton() {
  return (
    <Card>
      <CardHeader>
...
export function NetbookDetails({ serialNumber, web3Service }: NetbookDetailsProps) {
  ...
  const { isLoading: isTxLoading, error } = useWeb3();
  ...
  const stateInfo = getNetbookStateInfo(Number(netbook?.state ?? 0));
  const progressValue = ((Number(netbook?.state ?? 0) + 1) / 4) * 100;
